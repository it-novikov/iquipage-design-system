...
export async function mountPlanning(root,{adapter,project,onOpenTask,onCreateTask,onCreateRelease,onOpenBoard,viewState={}}) {
  ...
  return {reload:()=>controller.load(),controller,readyToLeave:()=>!opening&&!modal&&!switching&&(!temporal||temporal.readyToLeave()),
    destroy(){if(disposed)return;if(temporal&&!temporal.destroy())return false;viewState.scrollTop=scroll.scrollTop;disposed=true;clearTimeout(searchTimer);observer?.disconnect();unsubscribe?.();abort.abort();controller.destroy();if(root.firstElementChild===page){root.classList.remove('planning-root');page.remove();}}};
}
