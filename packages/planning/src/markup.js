...
export function groupBody(group,controller) {
  ...
  else if(group.loaded&&!group.busy&&!group.error)body=`<div class="pn-empty">${controller.filters.query||controller.filters.preparation!=='all'?'<p>Нет задач по этим условиям</p>':'<p>Задач пока нет</p>'}${controller.capabilities.createTask?ui.btn('Добавить задачу','ghost sm','plus','data-add'):''}</div>`;
