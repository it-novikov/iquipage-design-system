...
export function mountProjectNavigation(root,{current,profile,projects,onSwitch,onAccount,onSignOut,onSettings,onTeam}){
  ...
  async function switchProject(){
    ...
    dialog=formDialog({title:'Пространства и проекты',body:'<iq-remote-combobox label="Найти пространство или проект" placeholder="Название проекта или пространства"></iq-remote-combobox><p class="iq-helper">Выбор проекта меняет весь рабочий контекст. Ваши задачи остаются в своём проекте.</p>',submitLabel:'Открыть проект',
      canClose:()=>!switching,submit:async form=>{const id=form.querySelector('iq-remote-combobox').value;if(!id)throw Error('Выберите проект.');switching=true;
        try{const next=await onSwitch(id);if(next===false)throw Error('Сначала завершите открытое редактирование. Текущий проект не изменён.');if(!disposed&&next)update(next);return true;}finally{switching=false;}},
  ...
  root.addEventListener('click',event=>{if(event.target.closest('[data-context-switch]'))void switchProject();if(event.target.closest('[data-project-settings]'))onSettings?.();},{signal:events.signal});
  root.addEventListener('iq-action',event=>{if(event.detail.action==='account')onAccount?.();if(event.detail.action==='signout')onSignOut?.();if(event.detail.action==='team')onTeam?.();},{signal:events.signal});
  return {update,readyToLeave:()=>!switching&&!dialog,destroy(){if(switching)return false;disposed=true;dialog?.close();events.abort();root.replaceChildren();return true;}};
}
