...
export function quickFieldDialog({adapter,projectId,row,field,revision,onChanged,onClose}) {
  ...
  const operation=new PlanningOperation(adapter,projectId,state=> {
    ...
    modal.setBusy(busy);
    modal.form.querySelector('[data-pn-submit]').textContent=uncertain?'Проверить результат':'Сохранить';
  ...
  modal=formDialog({
    ...
    body:`<p>${esc(row.key)} — ${esc(row.title)}</p><fieldset class="pn-single-field">${control}</fieldset><p class="iq-helper">${help}</p>`,
