...
export interface PlanningRow {id:string;taskId:string;key:string;title:string;type:'task'|'bug'|'epic';priority:'normal'|'low'|'high'|'critical';preparation:'draft'|'ready';statusLabel:string;ownerLabel?:string;ownerId?:string;dueValue?:string|null;dateLabel?:string;depth:number;childrenCount:number;contextOnly:boolean;selectable:boolean;parentContext?:string|null;parentId?:string|null;outcome?:'open'|'accepted'|'cancelled';revision?:number}
...
export interface ViewState {deadlineMode?:'timeline'|'dates';filters?:Filters;collapsed?:string[];scrollTop?:number;mode?:'list'|'timeline'|'dates';temporal?:TemporalViewState}
