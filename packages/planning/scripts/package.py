"""Self-contained Frontend R1 source/fixture delivery. No secrets, user storage or font binaries."""
from pathlib import Path
import argparse, hashlib, json, shutil, tempfile, zipfile
root = Path(__file__).resolve().parents[3]
pn = root / 'packages/planning'
parser = argparse.ArgumentParser()
parser.add_argument('--out', type=Path, default=Path.home() / 'Downloads/Sprintique-Planning-Frontend-R1')
...
items = {}
roots = ['design-system', 'packages/maps', 'packages/planning', 'packages/work-list', '.agents/skills', 'agent-skills']
...
expected_html = sha(items['packages/planning/dist/Sprintique-Planning-Frontend-R1.html'])
...
marker = out / '.sprintique-planning-front-r1'
if any(out.iterdir()) and not marker.exists(): raise SystemExit('Output is not an owned Frontend R1 delivery directory')
marker.write_text('Frontend R1 fixture delivery, not production data\n')
manifest = ''.join(f'{sha(data)}  {name}\n' for name, data in sorted(items.items()))
items['MANIFEST.sha256'] = manifest.encode()
archive = out / 'Sprintique-Planning-Frontend-R1.zip'
with zipfile.ZipFile(archive, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zipped:
    for name, data in sorted(items.items()):
        info = zipfile.ZipInfo('Sprintique-Planning-Frontend-R1/' + name, (2026, 9, 12, 0, 0, 0))
...
with tempfile.TemporaryDirectory(prefix='sprintique-pn2-verify-') as temporary:
    ...
    extracted = Path(temporary) / 'Sprintique-Planning-Frontend-R1'
...
for source, name in [(pn / 'dist/Sprintique-Planning-Frontend-R1.html', 'Sprintique-Planning-Frontend-R1.html'), (pn / 'README.md', 'README.md'), (pn / 'evidence/verification.json', 'verification.json')]: shutil.copy2(source, out / name)
package_report = {'status': 'PASS_PACKAGE_INTEGRITY', 'scope': 'local Frontend R1 fixture and sources', 'files': len(items), 'manifestEntries': len(items)-1, 'archiveSha256': sha(archive.read_bytes()), 'backendIntegrated': False, 'productReady': False}
(out / 'package-verification.json').write_text(json.dumps(package_report, indent=2) + '\n')
(out / 'SHA256SUMS').write_text(''.join(f'{sha((out/name).read_bytes())}  {name}\n' for name in ['Sprintique-Planning-Frontend-R1.zip', 'Sprintique-Planning-Frontend-R1.html', 'verification.json', 'package-verification.json']))
