...
const report={release:'Board-B2.1.1',status:failure?'FAIL':'PASS',startedAt,finishedAt:new Date().toISOString(),sourceCommit:git.status===0?git.stdout.trim():null,sourceFingerprint:before.sha256,node:process.version,nodeTests,browserScenarios,checks,error:failure?.message,scope:'reusable-module-and-local-reference',attachmentCoreValidated:!failure,coverUiReady:!failure,coverIntentOrderingValidated:!failure,coverDecodeRecoveryValidated:!failure,httpAttachmentRoutesEnabled:true,threadPaginationValidated:!failure,productionReady:false,limitations:['No production Sprintique API/auth deployment','No live LLM credentials or provider calls','No multi-user collaboration','Real touch devices, screen readers and Safari not accepted','Uploads use a local reference store; production AV/CDR and host authorization require integration','Virtualization, provider integrations and production host delivery remain pending','Thread list is paginated; one opened discussion loads its complete message history, and reference queries are in-memory','Headless motion sample is not a real-device FPS acceptance']};
