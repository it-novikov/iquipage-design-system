...
const report={release:'Board-Attachment-Core-Review',status:failure?'FAIL':'PASS',startedAt,finishedAt:new Date().toISOString(),sourceCommit:git.status===0?git.stdout.trim():null,sourceFingerprint:before.sha256,node:process.version,nodeTests,browserScenarios,checks,error:failure?.message,scope:'reusable-module-and-local-reference',attachmentCoreValidated:!failure,coverUiReady:false,httpAttachmentRoutesEnabled:false,productionReady:false,limitations:['No production Sprintique API/auth deployment','No live LLM credentials or provider calls','No multi-user collaboration','Real touch devices, screen readers and Safari not accepted','Attachment core is validated; cover UI and reference-server attachment routes are not connected','Virtualization, provider integrations and production host delivery remain pending','Thread API loads project records; server cursor pagination remains pending','Headless motion sample is not a real-device FPS acceptance']};
