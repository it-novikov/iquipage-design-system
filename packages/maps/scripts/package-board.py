"""Package Board B1 only when current source and offline bytes match passing acceptance."""
from pathlib import Path
import hashlib, json, subprocess, zipfile
ROOT = Path(__file__).resolve().parents[1]
REPO = ROOT.parents[1]
OUT = REPO / 'deliveries/board-b1'
PREFIX = 'Sprintique-Board-B1/'
...
assert report['release'] == 'Board-B1', 'Wrong acceptance scope'
...
files['Sprintique-Board-B1.html'] = (ROOT / 'preview.html').read_bytes()
...
for p in sorted((ROOT / 'artifacts/board-b1').glob('*.png')):
    if 'failure' in p.name:
        continue
    files['packages/maps/evidence/board-b1/screenshots/' + p.name] = p.read_bytes()
manifest = ''.join(sha(data) + '  ' + name + '\n' for name, data in sorted(files.items()))
files['MANIFEST.sha256'] = manifest.encode()
OUT.mkdir(parents=True, exist_ok=True)
archive = OUT / 'Sprintique-Board-B1.zip'
...
(OUT / 'Sprintique-Board-B1.html').write_bytes(files['Sprintique-Board-B1.html'])
(OUT / 'Sprintique-Board-B1-Verification.json').write_text(json.dumps(report, ensure_ascii=False, indent=2) + '\n')
