...
export class MapsFeature {
  ...
  empty(){this.current=null;this.surface.innerHTML=`<div class="map-empty"><span class="map-empty-symbol">${icon('map',32)}</span><h2>Место для идей и решений</h2><p>Создайте постоянную карту для схем или сессию для совместного обсуждения.</p><div class="row">${button('templates','Выбрать основу','grid','primary')}${button('new','Чистая карта','plus','secondary')}</div></div>`;this.renderChrome();}
  async readyToLeave() {
  ...
  renderChrome(override='') {
    ...
    this.toolbar.innerHTML=`<div class="map-heading">${button('maps','Карты','map','ghost','aria-label="Открыть список карт"')}<span class="map-heading-separator">/</span><div class="map-title-wrap"><button type="button" class="map-title" data-map-action="rename" ${!m||disabled?'disabled':''}>${esc(m?.title||'Карты проекта')}</button><span class="map-save-label">${esc(override||(this.board?.saving?'Сохраняем…':m?this.storageLabel:'Выберите карту или создайте новую'))}</span></div>${m?`<span class="iq-badge ${m.status==='archived'?'outline':''}">${m.kind==='session'?'Сессия':'Постоянная'}</span>`:''}</div><div class="map-toolbar-actions">${button('templates','Шаблоны','grid')}${button('archive','Архив','archive')}${button('help','Как начать','info')}${m?button('more','Ещё','more'):''}${m&&!disabled?button(this.view==='workflow'?'add-step':'add',this.view==='workflow'?'Добавить шаг':'Записать мысли','plus','primary'):!m&&this.permissions.edit?button('new','Создать карту','plus','primary'):''}</div>`;
    if(!m){this.subbar.hidden=true;return;}this.subbar.hidden=false;
    const state=statusNames[m.status]||m.status;
    this.subbar.innerHTML=`<div class="map-view-group"><div class="iq-segmented" aria-label="Рабочая поверхность"><button type="button" data-map-action="canvas" aria-pressed="${this.view==='canvas'}">Карта</button><button type="button" data-map-action="workflow" aria-pressed="${this.view==='workflow'}">Сценарий действий${m.flow?'':' +'}</button></div><span class="map-state"><i></i>${esc(state)}</span>${m.kind==='session'&&m.status!=='archived'?`<button type="button" class="map-timer-label" data-map-action="session" aria-label="Этап и таймер сессии"></button>`:''}</div><div class="map-context-actions">${this.view==='workflow'?button('properties','Свойства','sliders')+button('validate','Проверить','check')+button('runs','Запуски','clock')+button('automations','Автоматизация','bolt')+(this.permissions.run&&m.status!=='archived'?button('run','Тест и запуск','play','secondary'):''):button('find','Найти','search')}${m.kind==='session'&&m.status!=='archived'&&!disabled?button(m.status==='draft'?'start':m.status==='paused'?'resume':'finish',m.status==='draft'?'Начать сессию':m.status==='paused'?'Продолжить':'Завершить',m.status==='active'?'check':'play','secondary'):''}${m.status==='archived'?button('continue','Создать продолжение','plus','secondary'):''}</div>`;
    this.updateTimer();
  }
  ...
  async handle(action,control) {
    ...
    const actions={maps:()=>this.mapBrowser(false),archive:()=>this.mapBrowser(true),templates:()=>this.templateBrowser(),new:()=>this.newMapDialog(),help:()=>this.help(),add:()=>this.board.command('bulk'),find:()=>this.board.command('search'),more:()=>this.more(),rename:()=>this.rename(),dismiss:()=>this.message(''),canvas:()=>this.switchView('canvas'),workflow:()=>this.switchView('workflow'),session:()=>this.sessionDialog(),finish:()=>this.finishDialog(),start:()=>this.changeSession('start'),resume:()=>this.changeSession('resume'),pause:()=>this.changeSession('pause'),continue:()=>this.continueMap(),export:()=>this.exportMap(false),'export-draft':()=>this.exportMap(true),'save-template':()=>this.saveTemplateDialog(),agent:()=>this.agentDialog(),import:()=>this.importMap(),fullscreen:()=>this.message('Карта уже занимает доступную область. Навигация платформы остаётся доступной.')};
  ...
  async switchView(view) {
    ...
    if(view==='workflow'&&!this.current.flow){
      ...
      return this.dialog({title:'Добавить сценарий действий?',description:'Карта и сценарий остаются связанными, но независимыми. Заметки не превращаются в автоматизацию сами по себе.',body:'<p>Начнём с проверяемой цепочки: собрать заметки → подготовить список → подтвердить → создать задачи → сохранить результат. Внешние сервисы не вызываются при создании.</p>',submitLabel:'Добавить сценарий',onSubmit:async()=>{await this.saveMetadata({...clone(this.current),flow:createMeetingFlow()});await this.switchView('workflow');}});
    }
  ...
  help() {
    return this.dialog({title:'Как работать с картами',description:'Начните с мысли. Структура и автоматизация подключаются, когда они нужны.',wide:true,body:`<div class="map-help-grid"><article><b>1. Запишите мысли</b><p>«Записать мысли» открывает отдельные карточки. Одна мысль — одна заметка. Их можно перемещать и редактировать.</p></article><article><b>2. Соберите структуру</b><p>Объедините заметки в области, добавьте фигуры и связи. Библиотека «Объекты» находится внизу карты.</p></article><article><b>3. Договоритесь о действии</b><p>Превратите заметку в действие или откройте сценарий. Проверяйте результат перед созданием задач.</p></article></div><div class="map-help-types"><p><b>Постоянная карта</b> — архитектура, путь пользователя, зависимости и решения. Она развивается вместе с проектом.</p><p><b>Сессия</b> — обсуждение, которое вы начинаете и завершаете. Итоги остаются в архиве; продолжение создаётся отдельно.</p></div><details class="iq-accordion"><summary>Клавиатура и перемещение ${icon('plus',16)}</summary><div><p>Пробел и перетаскивание — двигать карту. Shift и щелчок — выбрать несколько объектов. Enter — редактировать. Escape — отменить текущий ввод. Ctrl/⌘ + Z — отменить изменение. 0 — показать всю карту. Поиск и точные формы позволяют работать без перетаскивания.</p></div></details><p class="map-explanation">Навигация проекта находится над картой. Задачи, материалы и настройки не являются частью модуля и предоставляются платформой.</p>`});
  }
  ...
  more() {
    return this.dialog({title:'Действия с картой',body:`<div class="map-action-menu">${button('export','Экспортировать карту','download','secondary')}${button('import','Импортировать карту или шаблон','upload','secondary')}${this.current.status!=='archived'&&this.permissions.edit?button('save-template','Сохранить как шаблон','grid','secondary'):''}${button('continue','Создать отдельную копию','copy','secondary')}${this.view==='canvas'?button('agent','Предложение агента','sparkles','secondary'):''}${this.current.kind==='permanent'&&this.current.status!=='archived'&&this.permissions.edit?button('finish','Убрать в архив','archive','secondary'):''}</div>`,mount:el=>el.addEventListener('click',e=>{const action=e.target.closest('[data-map-action]')?.dataset.mapAction;if(action){el.close(true);this.handle(action).catch(error=>this.message(error.message,true));}})});
  }
