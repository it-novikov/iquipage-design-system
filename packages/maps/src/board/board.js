...
export async function mountTaskBoard(root,{repository,project,canEdit=true,viewState={}}){
  registerCore();root.classList.add('iq-task-board-root');let items=[],disposeBoard,closed=false,loading=0,dragging=false,queued=false,activeDialog=null,dragIntent=null;
  ...
  return {reload,readyToLeave:async()=>!activeDialog&&!pending.size,destroy(){
    ...
    persist();if(root.firstElementChild===ownedPage)root.classList.remove('iq-task-board-root');closed=true;loading++;cancelAnimationFrame(searchFrame);abort.abort();unsubscribe?.();disposeBoard?.();
  }};
}
