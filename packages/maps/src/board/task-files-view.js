...
import {openCoverPresets} from './cover-presets.js';
...
export function mountTaskFiles(root,{coverRoot,adapter,task,readOnly=false}){
  ...
  let locked=false,presetPicker,cropDialog,coverCrop=task.coverCrop||null;
  ...
  coverRoot.addEventListener('click',event=>{
    if(locked)return;
    if(event.target.closest('[data-add-cover]')&&editable){presetPicker?.destroy();presetPicker=openCoverPresets(file=>{if(closed||locked)return;if(file)cropNew(file);else root.querySelector('[data-cover-input]').click();});}
