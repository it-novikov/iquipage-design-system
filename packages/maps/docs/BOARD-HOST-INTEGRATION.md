...
Проверенная база: B2.1.1. COV-01/COV-02 закрыты повторной приёмкой: последнее
намерение выбора обложки не перезаписывается поздним ответом, а отсутствие
адаптера и ошибка декодирования имеют явные недоступные состояния.
...
Готовность preview наступает только после успешного decode. Поздние fetch/decode
не меняют новую карточку или новый выбор; URL устаревшего запроса освобождается.
