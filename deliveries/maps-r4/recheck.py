...
root = Path(tempfile.mkdtemp(prefix='maps-r4-recheck-'))
...
original = json.loads((OUT / 'IQUIPAGE-Maps-R4-Verification.json').read_text())
assert verification['status'] == 'PASS'
assert verification['sourceFingerprint'] == original['sourceFingerprint']
assert sha((module / 'preview.html').read_bytes()) == sha((OUT / 'IQUIPAGE-Maps-R4.html').read_bytes())
