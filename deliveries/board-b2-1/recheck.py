...
archive = OUT / 'Sprintique-Board-B2.1.zip'
sha = lambda data: hashlib.sha256(data).hexdigest()
root = Path(tempfile.mkdtemp(prefix='board-b2-1-recheck-'))
with zipfile.ZipFile(archive) as z:
    ...
    z.extractall(root)
base = root / 'Sprintique-Board-B2.1'
...
original = json.loads((OUT / 'Sprintique-Board-B2.1-Verification.json').read_text())
assert verification['status'] == 'PASS'
assert verification['sourceFingerprint'] == original['sourceFingerprint']
assert sha((module / 'preview.html').read_bytes()) == sha((OUT / 'Sprintique-Board-B2.1.html').read_bytes())
