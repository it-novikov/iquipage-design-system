...
root = Path(tempfile.mkdtemp(prefix='board-b1-recheck-'))
...
original = json.loads((OUT / 'Sprintique-Board-B1-Verification.json').read_text())
assert verification['status'] == 'PASS'
assert verification['sourceFingerprint'] == original['sourceFingerprint']
assert sha((module / 'preview.html').read_bytes()) == sha((OUT / 'Sprintique-Board-B1.html').read_bytes())
