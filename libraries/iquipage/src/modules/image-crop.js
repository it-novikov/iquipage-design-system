...
function cropRect(width,height,zoom=1,x=.5,y=.5,aspectRatio=1){
 if(![width,height,zoom,x,y].every(Number.isFinite)||width<=0||height<=0)throw new TypeError('Некорректная геометрия изображения');
 if(!Number.isFinite(aspectRatio)||aspectRatio<.1||aspectRatio>10)throw new RangeError('Соотношение сторон: 0.1–10');
...
class IqImageCrop extends HTMLElement{
 static get observedAttributes(){return ['disabled','readonly','eyebrow','title','subtitle','placeholder','aspect-ratio']}
 ...
 get value(){if(!this.image)return null;return cropRect(this.image.naturalWidth,this.image.naturalHeight,this.zoom,this.x,this.y,this.aspectRatio)}
 set value(v){
  if(!this.image||!v||![v.x,v.y,v.width,v.height].every(Number.isFinite)||v.width<=0||Math.abs(v.width/v.height-this.aspectRatio)>.001)throw new TypeError('Кадр: заданное соотношение сторон и конечные координаты');
  const w=this.image.naturalWidth,h=this.image.naturalHeight,z=Math.min(w,h*this.aspectRatio)/v.width;
 ...
 attributeChangedCallback(name){if(name==='aspect-ratio'){this.zoom=1;this.x=this.y=.5;if(this.pending){this.epoch++;this.requestAbort?.abort();this.pending=false;}}this.updateContent();if(['readonly','disabled'].includes(name)&&this.locked){this.stopGesture?.();if(this.pending){this.epoch++;this.requestAbort?.abort();this.pending=false;this.message='Ожидание отменено. Кадр сохранён.'}}this.paint()}
 connectedCallback(){
 ...
 async export({size=512,type='image/png',quality=.92}={}){
  ...
  const r=this.value,c=document.createElement('canvas');c.width=Math.max(1,Math.round(size*Math.min(1,this.aspectRatio)));c.height=Math.max(1,Math.round(size/Math.max(1,this.aspectRatio)));const ctx=c.getContext('2d');ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(this.image,r.x,r.y,r.width,r.height,0,0,c.width,c.height);
 ...
 paint(){if(!this.isConnected||!this.querySelector('canvas'))return;
  const stage=this.querySelector('.iq-crop-stage');stage.style.aspectRatio=String(this.aspectRatio);
  ...
  stage.classList.toggle('has-image',!!this.image);stage.setAttribute('aria-busy',String(this.loading));
