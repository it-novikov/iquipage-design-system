...
function bindBoard(board, onDrop, options = {}) {
    ...
    function flip(before) {
        ...
        for (const [n, a, b] of moves) {
            ...
            const anim = n.animate([{ transform: `translate(${dx}px,${dy}px)` }, { transform: 'none' }], { duration: motion.standard, easing: motion.ease });
    ...
    function begin(card, keyboard = false, x = 0, y = 0, pointerId = null) {
        ...
        ghost.innerHTML = `<div class="task-card-top"><span class="task-card-id">${card.querySelector('.task-card-id').textContent.replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]))}</span></div><div class="task-card-title">${card.querySelector('.task-card-title').textContent.replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]))}</div>${(card.querySelector('.task-card-footer')?.outerHTML||'')}`;
    ...
    function finish() {
        ...
        if (reduced())
            ...
            const anim = s.ghost.animate([{ transform: `translate3d(${from.left}px,${from.top}px,0) rotate(-1deg)`, opacity: 1 }, { transform: `translate3d(${destination.left}px,${destination.top}px,0) rotate(0)`, opacity: 1 }], { duration: motion.enter, easing: motion.spring, fill: 'forwards' });
            s.landing = anim;
