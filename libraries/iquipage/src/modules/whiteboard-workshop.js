 ...
 Object.assign(Klass.prototype,{
  ...
  renderLibrary(){
   ...
   if(this.libraryCategory==='notes'){
    html=noteEntries.filter(n=>this.allowedCreateTypes.includes(n[0])&&match(n.join(' '))).map(card).join('');
    if(this.allowedCreateTypes.includes('sticky')&&(!query||match('Несколько заметок')))html+=`<button type="button" class="wb-library-batch" data-wb-action="bulk" ${this.locked?'disabled':''}>${icon('plus',18)}<span><b>Несколько заметок</b><small>Каждая мысль — отдельная карточка</small></span></button>`;
   }else if(this.libraryCategory==='shapes'){
    for(const group of ['Основные','Данные','Действия','Переходы']){
     const options=this.allowedCreateTypes.includes('shape')?Object.values(Shapes.SHAPE_DEFS).filter(d=>d.group===group&&match(d.label+' '+d.group)):[];if(!options.length)continue;
    ...
    if(this.allowedCreateTypes.includes('image')&&match('Изображение PNG JPEG WebP'))html=card(['image','Изображение','PNG, JPEG, WebP · с устройства','image','sky']);
