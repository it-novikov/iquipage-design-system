...
export function formDialog({title,body,submitLabel='',submit,onMount,onClose=()=>{},canClose=()=>true,canSubmit=()=>true}) {
  ...
  form.addEventListener('submit',async e=>{
    e.preventDefault();if(busy||!submit||!canSubmit())return;fail('');setBusy(true);
    try{const keep=await submit(form,{fail,setBusy,close});setBusy(false);if(keep!==false)close();}
    catch(error){fail(error.message||'Изменение не сохранено.');setBusy(false);}
  });
  host.addEventListener('iq-close',()=>{onClose();host.remove();},{once:true});
  document.body.append(host);onMount?.(form,{setBusy,fail,close});host.show();setBusy(false);
