...
import {readImage} from './capture-ui.js';
...
export async function showCanvasInspector(feature,ids,{focusText=false}={}){
  ...
  // Selection may arrive before the newly added object has received its server revision.
  // Flush and await that exact save before capturing the inspector's compare-and-set base.
  feature.board?.flush();if(feature.savingPromise)await feature.savingPromise;
  if(!feature.alive||feature.board?.dirty)return;
  ...
  const root=feature.inspector,abort=new AbortController();let baseline='',saving=false,closed=false,imageSrc=object?.src||'',imageLoading=false;
  ...
  const value=()=>{const data=new FormData(form),v=Object.fromEntries(data);for(const select of form.querySelectorAll('iq-select'))v[select.getAttribute('name')]=select.value;if(editor)v.text=editor.value;if(object?.type==='image'){delete v.image;v.src=imageSrc;}return v;};
  baseline=JSON.stringify(value());form.querySelector('[data-replace-image]')?.addEventListener('click',()=>form.querySelector('[name=image]').click(),{signal:abort.signal});
  form.querySelector('[name=image]')?.addEventListener('change',async event=>{
    const file=event.target.files[0],status=form.querySelector('[data-image-status]');if(!file||imageLoading||readOnly)return;
    imageLoading=true;status.textContent='Проверяем изображение…';
    try{const src=await readImage(file);if(closed)return;imageSrc=src;status.textContent='Изображение готово. Сохраните изменения.';}
    catch(error){if(!closed)status.textContent=error.message;}finally{imageLoading=false;event.target.value='';}
  },{signal:abort.signal});
  const dirty=()=>!readOnly&&JSON.stringify(value())!==baseline;
  const close=async()=>{if(saving||imageLoading)return false;if(dirty()){let discarded=false;const confirm=dialog({title:'Закрыть свойства без сохранения?',description:'Изменения объекта ещё не сохранены.',submitLabel:'Не сохранять',onSubmit:async()=>{discarded=true;}});await new Promise(resolve=>confirm.addEventListener('iq-close',resolve,{once:true}));if(!discarded)return false;}closed=true;abort.abort();if(!motionReduced()){await root.animate([{opacity:1,transform:'translateX(0)'},{opacity:0,transform:'translateX(16px)'}],{duration:180,easing:'ease-out'}).finished.catch(()=>{});}root.hidden=true;root.replaceChildren();delete root.dataset.canvasInspector;if(feature.canvasInspector?.id===id)feature.canvasInspector=null;return true;};
  feature.canvasInspector={id,close,dirty,destroy:()=>{closed=true;abort.abort();root.replaceChildren();root.hidden=true;}};
  ...
  form.addEventListener('submit',async event=>{
    event.preventDefault();if(readOnly||saving||closed||imageLoading)return;const error=form.querySelector('[role=alert]');error.hidden=true;saving=true;
