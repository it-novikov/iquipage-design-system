import {captureDialog} from './capture-ui.js'; import {showMapHistory} from './map-history.js';
import {showCanvasInspector,leaveCanvasInspector} from './canvas-inspector.js';
import {motionReduced} from '@iquipage/web/core';
import {registerWhiteboard,validateWhiteboard} from '@iquipage/web/whiteboard';
...
export class MapsFeature {
  ...
  async handle(action,control) {
    if(action==='history')return showMapHistory(this);if(action==='toggle-session'){if(!(await leaveCanvasInspector(this)))return;this.sessionOpen=!this.sessionOpen;this.renderSessionCard();return;}
  ...
  async newMapDialog(template=null) {
    ...
    this.dialog({title:template?'Создать карту из шаблона':'Новая карта',body:`${input('title','Название',template?.title||'Новая карта',{required:true,maxlength:160})}${this.uiCapabilities.sessions?select('kind','Как будем работать',template?.kind||'permanent',[['permanent','Постоянная — схемы и долгосрочная работа'],['session','Сессия — обсуждение с завершением']]):''}${template?`<p class="map-explanation">${esc(template.description||'')} Новая карта не меняет шаблон и другие карты.</p>`:''}`,submitLabel:'Создать карту',onSubmit:async values=>{
  ...
  saveTemplateDialog() {
    ...
    return this.dialog({title:'Сохранить свой шаблон',description:'Сохраняется структура, а не голоса, запуски или подключения. Назовите её по задаче пользователя.',body:`${input('title','Название шаблона','',{required:true,maxlength:160,placeholder:'Например: подготовить встречу с клиентом'})}${textarea('description','Для чего он нужен','',2)}${textarea('when','Когда использовать','',2)}${select('scope','Кому доступен','project',[['personal','Только мне'],['project','В этом проекте'],['workspace','Во всём рабочем пространстве']])}${ids.length?check('selection','Только выбранный фрагмент',true):''}${check('includeContent',this.repository.capabilities?.templateImages===false?'Сохранить текст заметок':'Сохранить текст заметок и изображения',false)}<p class="map-explanation">${this.repository.capabilities?.templateImages===false?'Приватные изображения и ссылки на задачи не входят в общий шаблон. ':''}По умолчанию содержание заметок заменяется подсказками. Названия областей и фигур сохраняются — проверьте, нет ли в них личных данных.</p>`,submitLabel:'Сохранить шаблон',onSubmit:async values=>{
      if(!(await this.readyToLeave()))throw Error('Сначала завершите изменение карты.');
      const t=templateFromMap(this.current,{title:values.get('title'),description:values.get('description'),when:values.get('when'),scope:values.get('scope'),selectedIds:values.has('selection')?ids:null,includeContent:values.has('includeContent'),includeImages:this.repository.capabilities?.templateImages!==false});
  ...
  rename() {
    if(!this.current)return;return this.dialog({title:'Название карты',body:input('title','Название',this.current.title,{required:true,maxlength:160}),submitLabel:'Сохранить',onSubmit:async values=>this.saveMetadata({...clone(this.current),title:values.get('title').trim()})});
  }
  more() {
