import {icon,escapeHTML as esc,registerCore} from '@iquipage/web/core';
export {icon,esc};
...
let fieldSequence=0;
export const input=(name,label,value='',{type='text',placeholder='',required=false,maxlength=240}={})=>{
  const id=`maps-field-${++fieldSequence}`;
  return `<div class="iq-field"><label class="iq-control-label" for="${id}">${esc(label)}</label><span class="iq-input-shell"><input id="${id}" name="${esc(name)}" type="${type}" value="${esc(value)}" placeholder="${esc(placeholder)}" ${required?'required':''} maxlength="${maxlength}"></span></div>`;
};
export const textarea=(name,label,value='',rows=4)=>{
  const id=`maps-field-${++fieldSequence}`;
  return `<div class="iq-field"><label class="iq-control-label" for="${id}">${esc(label)}</label><span class="iq-input-shell"><textarea id="${id}" name="${esc(name)}" rows="${rows}" maxlength="50000">${esc(value)}</textarea></span></div>`;
};
