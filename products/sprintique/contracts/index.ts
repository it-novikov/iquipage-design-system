...
export const TagInput = z.strictObject({name: z.string().trim().min(1).refine(v => [...v].length <= 24), tone: z.enum(['neutral','blue','purple','green','amber','red']), archivedAt: z.iso.datetime().nullable().default(null)});
...
export interface Project {id:string;workspaceId:string;name:string;slug:string;key:string;role:'reader'|'editor'|'admin';workspaceName:string}
