import {z} from 'zod';
import {Id,Revision,DateOnly} from './index.js';

export const PLANNING_POLICY = 'planning-r2.1';
...
export const PlanningCommand = z.discriminatedUnion('kind',[
  ...
  z.strictObject({kind:z.literal('bulk'),selection:Selection,fields:z.strictObject({
    ...
    plannedStart:DateOnly.nullable().optional(),due:DateOnly.nullable().optional()
  ...
  z.strictObject({kind:z.literal('close'),releaseId:Id,accepted:Selection,destination}),
  z.strictObject({kind:z.literal('cancel'),releaseId:Id,destination})
]);
...
export interface PlanningRow {
  ...
  plannedStart:string|null;due:string|null;assignmentMode:'inherit'|'assigned'|'none';releaseId:string|null;
...
export const MilestoneWrite=z.strictObject({baseRevision:Revision,value:z.strictObject({title:z.string().trim().min(1).max(240),date:DateOnly.nullable()})});
const EntityRef=z.strictObject({kind:z.enum(['task','release','milestone']),id:Id});
export const ConstraintWrite=z.strictObject({baseRevision:Revision,value:z.strictObject({source:EntityRef,target:EntityRef,relation:z.enum(['FS','SS','FF','SF']),lagDays:z.number().int().min(-36500).max(36500)})});
export interface Milestone {id:string;title:string;date:string|null;revision:number}
