...
export function registerLimits(app:FastifyInstance){
  ...
  app.addHook('onRequest',async r=>{
    // Readiness is anonymous and probes real dependencies, so it shares the per-address window.
    const readiness=r.url==='/ready'||r.url.startsWith('/ready?');
    if(!readiness&&!r.url.startsWith('/api/')&&!r.url.startsWith('/auth/'))return;
    ...
    // A container healthcheck must never be shed by the concurrency admission below.
    if(readiness)return;
