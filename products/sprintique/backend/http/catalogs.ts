  ...
  for(const collection of ['tags','releases'] as const){
    ...
    app.put(`/api/v1/projects/:projectId/${collection}/:id`,request=>authenticated(request,async(tx,actor)=>{
      ...
      return idempotent(tx,actor,projectId,collection+':'+id,mutationKey(request),input,async()=>{
        ...
        // Legacy UI sends a date-shaped archive intent; the canonical clock is always server-owned.
        if('tone' in value)await tx.query(`INSERT INTO app.tags(project_id,id,name,tone,archived_at,revision) VALUES($1,$2,$3,$4,CASE WHEN $5 THEN clock_timestamp() END,1) ON CONFLICT(project_id,id) DO UPDATE SET name=$3,tone=$4,archived_at=CASE WHEN $5 THEN coalesce(app.tags.archived_at,clock_timestamp()) END,revision=app.tags.revision+1`,[projectId,id,value.name,value.tone,!!value.archivedAt]);
        else await tx.query(`INSERT INTO app.releases(project_id,id,name,status,target_date,deadline,archived_at,revision) VALUES($1,$2,$3,$4,$5,$5,CASE WHEN $6 THEN clock_timestamp() END,1) ON CONFLICT(project_id,id) DO UPDATE SET name=$3,target_date=$5,deadline=$5,archived_at=CASE WHEN $6 THEN coalesce(app.releases.archived_at,clock_timestamp()) END,revision=app.releases.revision+1`,[projectId,id,value.name,value.status,value.targetDate,!!value.archivedAt]);
