...
import {provisionTaskSettings} from './workspace.js';
import {manageProject} from './membership.js';
import {workspaceLock} from './project-profile.js';

export async function sessionInfo(tx:Transaction,actor:Actor){
  const projects=(await tx.query<Project>(`SELECT p.id,p.workspace_id AS "workspaceId",p.name,p.slug,p.key,p.avatar_asset_id AS "avatarAssetId",m.role,w.name AS "workspaceName" FROM app.projects p JOIN app.project_members m ON m.project_id=p.id AND m.principal_id=$1 JOIN app.workspaces w ON w.id=p.workspace_id ORDER BY w.name,p.name`,[actor.id])).rows;
...
export async function createProject(tx:Transaction,actor:Actor,input:{workspaceId:string;name:string;slug:string;key:string;timezone?:string|null}){
  ...
  await workspaceLock(tx,actor,input.workspaceId);
  ...
  await provisionTaskSettings(tx,id,actor.id);
...
export async function issueAgent(tx:Transaction,actor:Actor,projectId:string,input:{name:string;capabilities:Capability[];expiresInSeconds:number}){
  ...
  const count=(await tx.query<{n:number}>('SELECT count(*)::int n FROM app.project_members WHERE project_id=$1',[projectId])).rows[0]!.n;
