...
`TRUST_PROXY` must be set when the API runs behind the TLS reverse proxy: a hop count (usually `1`) or the exact comma-separated proxy addresses. Per-address request and login limits then apply to the real client; without it every user shares the proxy address and one client can exhaust the login window for everyone. Leave it unset only when clients connect directly. Responses from an HTTPS origin carry HSTS, `Cross-Origin-Opener-Policy: same-origin` and a restrictive `Permissions-Policy`.
...
The drill creates private custom-format dumps of application and identity databases, copies referenced object variants with checksums, and fingerprints canonical tables. It restores into a new isolated PostgreSQL container and S3 prefix, compares table fingerprints, authenticates a restored session and downloads restored private files; anonymous reads must fail. The restore container holds real private data on a published loopback port, so every TCP login uses a fresh per-run secret and SCRAM; it is never started with trust authentication. Container teardown runs even when a client or object cleanup step fails. It rechecks the source fingerprints at the end. Temporary restored objects/container/anonymous volume are removed; the private checkpoint remains in `output/infra/backups/<id>/`.
